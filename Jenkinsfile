node {
    ...
    stage('Build Custom Image and Publish Image to Registry'){
        sh 'echo $PATH'
        def customImage = docker.build("${env.DOCKER_ELASTIC_APP_REPO}")
    ...
    stage ('Deploy or Upgrade Helm Chart for Kafka Consumer Webapp') {
        ...
        helm upgrade --install --wait --set image.repository=${env.DOCKER_ELASTIC_APP_REPO},image.tag=${commit_id},configVariables.kafkaHost=${env.KAFKA_HOST},configVariables.elasticHost=${env.ELASTIC_HOST},configVariables.elasticPort=${ELASTIC_PORT} kafka-elastic-app ./helm-chart/
        """
        }
    }
}